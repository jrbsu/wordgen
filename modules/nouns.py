...
def noun_with_suffix(current_word, base_words, suffix):
    ...
    return [
        utils.stress(re.sub(r'ë$', 'e', erg) + suffix),
        utils.stress(re.sub(r'ë$', 'e', acc) + suffix),
        utils.stress(re.sub(r'ë$', 'e', dat) + suffix),
        utils.stress(re.sub(r'ë$', 'e', abs) + suffix)
    ]
...
def get_noun_data(current_word, gender, is_special):
    ...
    erg = utils.stress(erg)
    abs = utils.stress(abs)
    if len(acc) <= 2 and gender != "solar":
        acc = utils.stress(re.sub(r'ë$', 'e', acc))
        dat = utils.stress(re.sub(r'ë$', 'e', dat))
    else:
        acc = utils.stress(acc)
        dat = utils.stress(dat)
    ...
    if re.search(r'[aeëio]', noun_end):
        row_data["POSS_1S"][case_dict["erg"]] = utils.stress(erg + "n")
        if len(acc) <= 2:
            row_data["POSS_1S"][case_dict["acc"]] = utils.stress(re.sub(r'ë$', 'e', acc) + "n")
        else:
            row_data["POSS_1S"][case_dict["acc"]] = utils.stress(acc + "n")
        row_data["POSS_1S"][case_dict["dat"]] = utils.stress(dat + "n")
    elif re.search(r'[fvszcxwljr]', noun_end):
        row_data["POSS_1S"][case_dict["erg"]] = utils.stress(erg + "ën")
        row_data["POSS_1S"][case_dict["abs"]] = utils.stress(abs + "ën")
    elif gender == "solar":
        row_data["POSS_1S"][case_dict["abs"]] = utils.stress(abs + "ën")
    ...
    if gender == "solar":
        row_data["POSS_3S"][case_dict["acc"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "an")

    # POSS 1P
    row_data["POSS_1P"] = noun_with_suffix(current_word, base_words, "ran")
    row_data["POSS_1P"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "eran")
    if not re.search(r'[aeëio]', noun_end):
        row_data["POSS_1P"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "eran")
    ...
    if gender == "lunar":
        row_data["POSS_3P"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "anon")
    ...
    row_data["CASE_GEN"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "ek")
    if not re.search(r'[aeëio]', noun_end):
        row_data["CASE_GEN"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "ek")

    # CASE INE
    row_data["CASE_INE"] = noun_with_suffix(current_word, base_words, "tta")
    row_data["CASE_INE"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "itta")
    if not re.search(r'[aeëio]', noun_end):
        row_data["CASE_INE"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "itta")
    ...
    row_data["CASE_INSTR"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "eno")
    if not re.search(r'[aeëio]', noun_end):
        row_data["CASE_INSTR"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "eno")

    # NUM_NONE
    row_data["NUM_NONE"] = noun_with_suffix(current_word, base_words, "kke")
    row_data["NUM_NONE"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "ikke")
    if not re.search(r'[aeëio]', noun_end):
        row_data["NUM_NONE"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "ikke")
    ...
    row_data["NUM_DUAL"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "ede")
    if not re.search(r'[aeëio]', noun_end):
        row_data["NUM_DUAL"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "ede")

    # NUM_PLU
    # Might work. Needs testing.
    # row_data["NUM_PLU"] = noun_with_suffix(current_word, base_words, "z")
    # row_data["NUM_PLU"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "ez")
    # if not re.search(r'[aeëio]', noun_end):
    #     row_data["NUM_PLU"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "ede")
    
    if re.search(r'[aeëio]', noun_end):
        row_data["NUM_PLU"] = [
            utils.stress(re.sub(r'ë$', 'e', erg) + "z"),
            utils.stress(acc + "z"),
            utils.stress(dat + "z"),
            utils.stress(re.sub(r'ë$', 'e', abs) + "ez")
        ]
    else:
        row_data["NUM_PLU"] = [
            utils.stress(re.sub(r'ë$', 'e', erg) + "ez"),
            utils.stress(acc + "z"),
            utils.stress(dat + "z"),
            utils.stress(re.sub(r'ë$', 'e', abs) + "ez")
        ]
    ...
    row_data["NUM_PAU"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "orë")
    if not re.search(r'[aeëio]', noun_end):
        row_data["NUM_PAU"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "orë")

    # NUM_COLL
    row_data["NUM_COLL"] = noun_with_suffix(current_word, base_words, "ca")
    row_data["NUM_COLL"][case_dict["abs"]] = utils.stress(re.sub(r'ë$', 'e', abs) + "oca")
    if not re.search(r'[aeëio]', noun_end):
        row_data["NUM_COLL"][case_dict["erg"]] = utils.stress(re.sub(r'ë$', 'e', erg) + "oca")
...
def noun_table(current_word, gender, is_special):
    ...
    noun_table += make_row("CASE_INE", colspan=2, tooltip="within, in, inside")
    noun_table += make_row("CASE_ADE", colspan=2, tooltip="at, upon, on")
    noun_table += make_row("CASE_ABL", colspan=2, tooltip="away from, from")
    noun_table += make_row("CASE_ALL", colspan=2, tooltip="towards, to")
    noun_table += make_row("CASE_INSTR", colspan=2, tooltip="using, by means of")
